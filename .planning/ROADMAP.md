...
A rebuild of a working v0 — the concept is proven, the architecture is not. Six phases move from module skeleton and bug fixes through auth, core messaging, profile/settings/themes, invite flow, and finally a proper test suite. Every phase delivers a coherent, verifiable capability. The transport abstraction thread runs through everything: Phase 1 creates the interface, every subsequent phase uses it, and the final result is a codebase where swapping Supabase for a mesh transport is a one-line change.
...
- [ ] **Phase 4: Profile + Settings + Themes** - Full profile screen, settings screen, CSS theme system (Postdesk + Terminal)
- [ ] **Phase 5: Invite Flow** - Invite generation, landing screen, atomic invite acceptance
- [ ] **Phase 6: Tests** - Jest tests for all pure modules + mock-transport integration tests
...
### Phase 4: Profile + Settings + Themes
**Goal**: Users have a full profile screen showing their @handle, display name, and avatar initial; a settings screen with theme picker and sign-out; and a CSS theme system delivering at least two themes (Postdesk — warm parchment/postal aesthetic; Terminal — existing dark amber). Theme preference is persisted to the user's Supabase profile and applied on load.
**Depends on**: Phase 3
**Requirements**: PROF-06, PROF-07, THEME-01, THEME-02, THEME-03, THEME-04
**Success Criteria** (what must be TRUE):
  1. User can view their profile (@handle, display name, avatar initial) from the inbox
  2. User can open settings and see theme options
  3. Selecting a theme applies it instantly, no reload required
...
| 4. Profile + Settings + Themes | 0/TBD | Not started | - |
| 5. Invite Flow | 0/TBD | Not started | - |
